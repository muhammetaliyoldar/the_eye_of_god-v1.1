...
TARGET_VIDEO_PATH = r"C:\Users\muham\PycharmProjects\the_eye_of_god-v1.1\show\lisenceplate-trafficlight-output\lisenceplate-trafficlight-output.mp4"
...
def main():
    ...
    with sv.VideoSink(TARGET_VIDEO_PATH, video_info) as sink:
        for frame in frame_generator:
            ...
            if frame_count <= 3:
                print(f"Green light indices: {green_light_indices}")
            ...
            cv2.putText(
                annotated_frame, 
                "THE EYE OF GOD - Plaka ve Trafik Isigi Tespiti", 
                (20, 30), 
            ...
            if hasattr(plate_detections, 'xyxy') and plate_detections.xyxy is not None and len(plate_detections) > 0:
                for i, (xyxy, confidence, class_id, tracker_id) in enumerate(
                    ...
                    # Prepare label
                    labels.append(f"Lisence Plate #{tracker_id}")
                ...
                annotated_frame = label_annotator.annotate(annotated_frame, plate_detections, labels)
            ...
            for i, box in enumerate(result.boxes.xyxy):
                try:
                    ...
                    if 'light' in class_name and confidence > 0.3:
                        ...
                        # Print debug info
                        if frame_count <= 5:
                            print(f"Drawing {class_name}: Box={x1},{y1},{x2},{y2}, Confidence={confidence:.2f}")
                except Exception as e:
            ...
            cv2.putText(legend, "Renk Kodlari:", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
            ...
            # Display frame
            cv2.imshow("THE EYE OF GOD - Plaka ve Trafik Isigi Tespiti", display_frame)
